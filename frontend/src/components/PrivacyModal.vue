    ...
    <b-modal v-model="showModal" centered no-header-close @hide="hideModal">
      ...
      <div>
        <b-form-textarea
          ...
          disabled
        />
        <UiToastEditorWrapper :initial-value="currentText" :none-clickable="true" />
        <div class="d-flex justify-content-between">
          <b-button
            class="toggle-button"
            @click="
              addWordToList('company');
              $event.target.blur();
            "
          >
        ...
        <div class="d-flex justify-content-between mt-3">
          <div class="confidential-words-div">
            <div v-for="(data, index) in confidentialWords.keys()" :key="data">
              <div
              ...
              >
                ...
                <b-button
                  ...
                  size="sm"
                  @click="
                    confidentialWords.delete(data);
                    caseSensitiveList = caseSensitiveList.filter(
      ...
      <div v-if="isDescription" class="my-2 d-inline-flex">
        <div class="position-relative mb-2">
          <input
            ...
            value="english"
          />
          <label id="english-label" for="english" class="position-absolute">{{
            t("general.aiFeature.privacyModal.labels.english")
          }}</label>
        </div>
        <div class="position-relative mb-2">
          <input
            ...
            value="german"
          />
          <label id="german-label" for="german" class="position-absolute">{{
            t("general.aiFeature.privacyModal.labels.german")
          }}</label>
        </div>
      </div>
      <template #footer>
  ...
  methods: {
    addWordToList(type: string) {
      ...
      if (
        ...
        !this.caseSensitiveList.includes(selection.toString().trim().toLowerCase()) &&
        (this.currentText.includes(selection.toString().trim()) ||
          this.currentTitle.includes(selection.toString().trim()))
      ) {
    ...
    submitIssue() {
      if (this.isDescription) {
        this.$emit("sendGPTRequest", {
          ...
          confidentialData: this.confidentialWords,
