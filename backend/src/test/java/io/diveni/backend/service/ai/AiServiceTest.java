...
public class AiServiceTest {
  ...
  public void improveTitle() {
    ...
    ResponseEntity<String> mockedResponse = new ResponseEntity<>(jsonReturnValue,mockedHeaders, HttpStatusCode.valueOf(200));

    when(aiService.improveDescription(new UserStory("1", "TEST", "TEST", "3", true))).thenReturn(mockedResponse);

    ResponseEntity<String> returnResponse = aiService.improveDescription(new UserStory("1", "TEST", "TEST", "3", true));
  ...
  public void improveDescription() {
    ...
    assertEquals("{'improved_description' : 'test', 'improved_acceptance_criteria' : '* TEST \n, * TEST 2 \n'}", returnResponse.getBody());
  }
}
