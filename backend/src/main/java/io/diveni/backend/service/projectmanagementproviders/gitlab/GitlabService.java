...
public class GitlabService implements ProjectManagementProviderOAuth2 {
    ...
    public List<Project> getProjects(String tokenIdentifier) {
        ...
        try {
            ResponseEntity<String> responseUser =
                    executeRequest("https://gitlab.com/api/v4/user", HttpMethod.GET, accessTokens.get(tokenIdentifier), null);
            ...
            ResponseEntity<String> response =
                    executeRequest("https://gitlab.com/api/v4/users/" + userName + "/projects",
                            HttpMethod.GET,
    ...
    public List<UserStory> getIssues(String tokenIdentifier, String projectName) {
        ...
        try{
            ResponseEntity<String> response =
                    executeRequest("https://gitlab.com/api/v4/projects/" + projectIDs.get(projectName) + "/issues?state=opened",
                            HttpMethod.GET,
                            accessTokens.get(tokenIdentifier),
                            null);
            JsonNode nodeProjects = new ObjectMapper().readTree(response.getBody());
            for (JsonNode node : nodeProjects) {
              String estimation = node.get("time_stats").get("human_time_estimate").asText().replaceAll("[^\\d.]", "");
    ...
    public void updateIssue(String tokenIdentifier, UserStory story) {
        ...
        try {
          executeRequest(
            "https://gitlab.com/api/v4/projects/" + projectIDTokenID.get(tokenIdentifier) +"/issues/" + story.getId(),
            HttpMethod.PUT,
            accessTokens.get(tokenIdentifier),
    ...
    private void updateEstimationOfIssue(String tokenIdentifier, UserStory story) {
        ...
        try {
            executeRequest(
              "https://gitlab.com/api/v4/projects/" + projectIDTokenID.get(tokenIdentifier) + "/issues/" + story.getId() + "/time_estimate",
              HttpMethod.POST,
              accessTokens.get(tokenIdentifier),
              content
            );
    ...
    public void deleteIssue(String tokenIdentifier, String issueID) {
        ...
        try {
          executeRequest(
            "https://gitlab.com/api/v4/projects/" + projectIDTokenID.get(tokenIdentifier) +"/issues/" + issueID,
            HttpMethod.PUT,
            accessTokens.get(tokenIdentifier),
            new Gson().toJson(content)
          );
